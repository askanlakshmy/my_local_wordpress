...
! defined( 'WPCBN_VERSION' ) && define( 'WPCBN_VERSION', '1.2.4' );
		...
		if ( ! class_exists( 'WPCleverWpcbn' ) && class_exists( 'WC_Product' ) ) {
			class WPCleverWpcbn {
				...
				function __construct() {
					...
					add_action( 'before_woocommerce_init', function () {
						if ( class_exists( '\Automattic\WooCommerce\Utilities\FeaturesUtil' ) ) {
							FeaturesUtil::declare_compatibility( 'custom_order_tables', WPCBN_FILE );
						}
					} );
				}
				...
				function single_shortcode( $atts ) {
					...
					$btn_class = apply_filters( 'wpcbn_btn_single_class', 'wpcbn-btn wpcbn-btn-single button alt', $atts );
